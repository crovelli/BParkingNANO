...
public:
  ...
  explicit ElectronMerger(const edm::ParameterSet &cfg):
    ...
    nnId_src_{ consumes<edm::ValueMap<float>>( cfg.getParameter<edm::InputTag>("nnId") )},
...
private:
  ...
  const edm::EDGetTokenT<edm::ValueMap<float>> nnId_src_;
...
void ElectronMerger::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const & iSetup) const {
  ...
  edm::Handle<edm::ValueMap<float> > nnId;  
  evt.getByToken(nnId_src_, nnId);
